...
class Export extends CI_Controller {
    ...
    public function xls_rpt_attendance_personnel_monthly($personnel = NULL, $year = NULL, $month = NULL) {
        ...
        //SUMMARY TABLE CONTENT VALUE
        $objPHPExcel->getActiveSheet()
            ->setCellValue($this->xls_inc($cell, 'C', 3), 'Total Hari Kerja')   
            ->setCellValue($this->xls_inc($cell, 'C', 5), $col4);
        //SUMMARY TABLE CONTENT CELL
        $objPHPExcel->getActiveSheet()->mergeCells($this->xls_inc($cell, 'C', 3).':'.$this->xls_inc($cell, 'C', 4));
        //SUMMARY TABLE CONTENT STYLE
        $objPHPExcel->getActiveSheet()->getStyle($this->xls_inc($cell, 'C', 3).':'.$this->xls_inc($cell, 'C', 4))->applyFromArray($styleThinBlackBorderOutline);
    ...
    public function xls_rpt_attendance_prsn_mnth_in_dept($dept_id = NULL, $year = NULL, $month = NULL) {
        ...
        foreach ($arr_prsn as $personnel => $prsn_name) {
            ...
            $col4 = empty($sa)?'':$sa->sum_hari_kerja;
            //SUMMARY TABLE CONTENT VALUE
            $objPHPExcel->getActiveSheet()
                ->setCellValue($this->xls_inc($cell, 'C', 3), 'Total Durasi Keterlambatan')   
                ->setCellValue($this->xls_inc($cell, 'C', 5), $col1);
    ...
    public function xls_rpt_attendance_department_yearly($dept_id = NULL, $year = NULL, $direct_download = 1) {
        ...
        foreach ($arr_prsn as $prsn_id => $prsn_name) {
            ...
            $objPHPExcel->getActiveSheet()->mergeCells($cell.':'.$this->xls_inc($cell, 'C', 4));
            $objPHPExcel->getActiveSheet()->getStyle($cell.':'.$this->xls_inc($cell, 'C', 4))->applyFromArray($styleThinBlackBorderOutline);
            $objPHPExcel->getActiveSheet()->getStyle($cell.':'.$this->xls_inc($cell, 'C', 4))->getFont()->setBold(true);
            $objPHPExcel->getActiveSheet()->getStyle($cell.':'.$this->xls_inc($cell, 'C', 4))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue($this->xls_inc($cell, 'C', 1), 'Hari Kerja')
                ->setCellValue($this->xls_inc($cell, 'C', 2), 'Terlambat')
                ->setCellValue($this->xls_inc($cell, 'C', 3), 'Durasi Terlambat')
                ->setCellValue($this->xls_inc($cell, 'C', 4), 'Keterangan');
            //COL STYLE
            $objPHPExcel->getActiveSheet()->getStyle($cell)->applyFromArray($styleThinBlackBorderOutline);
            $objPHPExcel->getActiveSheet()->getStyle($this->xls_inc($cell, 'C', 1))->applyFromArray($styleThinBlackBorderOutline);
            $objPHPExcel->getActiveSheet()->getStyle($this->xls_inc($cell, 'C', 2))->applyFromArray($styleThinBlackBorderOutline);
            ...
            $objPHPExcel->getActiveSheet()->getStyle($this->xls_inc($cell, 'C', 4))->applyFromArray($styleThinBlackBorderOutline);
            $objPHPExcel->getActiveSheet()->getStyle($cell.':'.$this->xls_inc($cell, 'C', 4))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $objPHPExcel->getActiveSheet()->getStyle($cell.':'.$this->xls_inc($cell, 'C', 4))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            ...
            for ($inc_month = 1; $inc_month <= 12; $inc_month++) {
                ...
                if ($sa != NULL) {
                    ...
                    $total_person_sum_hari_kerja    = $total_person_sum_hari_kerja+$col5;
                } else {
                    $col1 = 0;
                    $col2 = 0;
                    $col3 = 0;
                ...
                $objPHPExcel->getActiveSheet()
                    ->setCellValue($cell, $col1)
                    ->setCellValue($this->xls_inc($cell, 'C', 1), $col5)
                    ->setCellValue($this->xls_inc($cell, 'C', 2), $col2)
                    ->setCellValue($this->xls_inc($cell, 'C', 3), $col3)
                    ->setCellValue($this->xls_inc($cell, 'C', 4), $col4);
                ...
                $objPHPExcel->getActiveSheet()->getStyle($cell.':'.$this->xls_inc($cell, 'C', 4))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                $objPHPExcel->getActiveSheet()->getStyle($this->xls_inc($cell, 'C', 4))->getAlignment()->setWrapText(true);
                ...
                if ($sk == NULL) {
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle($cell . ":" . $this->xls_inc($cell, 'C', 4))->applyFromArray( $style_grey );
                } else {
                    $objPHPExcel->getActiveSheet()->getStyle($cell . ":" . $this->xls_inc($cell, 'C', 4))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
                    $objPHPExcel->getActiveSheet()->getStyle($cell . ":" . $this->xls_inc($cell, 'C', 4))->getFill()->getStartColor()->setARGB(PHPExcel_Style_Color::COLOR_WHITE);
                }
            }
            ...
            $objPHPExcel->getActiveSheet()
                    ...
                    ->setCellValue($this->xls_inc($cell, 'C', 1), $total_person_sum_hari_kerja)
                    ->setCellValue($this->xls_inc($cell, 'C', 2), $total_person_sum_is_late)
                    ->setCellValue($this->xls_inc($cell, 'C', 3), $total_person_jam.':'.$total_person_menit)
                    ->setCellValue($this->xls_inc($cell, 'C', 4), $str_total_keterangan);
            ...
            $objPHPExcel->getActiveSheet()->getStyle($cell.':'.$this->xls_inc($cell, 'C', 4))->applyFromArray($styleCustom);
